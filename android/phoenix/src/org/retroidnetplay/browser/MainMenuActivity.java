...
public class MainMenuActivity extends PreferenceActivity {
		...
		if (!prefs.getBoolean("first_time_refreshrate_calculate", false)) {
			...
			if (!detectDevice(false)) {
				AlertDialog.Builder alert = new AlertDialog.Builder(this)
						.setTitle("Welcome to RetroidNetplay")
						.setMessage(
								"This is your first time starting up RetroidNetplay. RetroidNetplay will now be preconfigured for the best possible gameplay experience.")
	...
	public void updateConfigFile() {
		...
		boolean netplayEnabled = prefs.getBoolean("netplay_enable", false); 
		if (netplayEnabled) {
			config.setBoolean("netplay_enable", true);
			config.setBoolean("is_client", prefs.getBoolean("is_client", false));
			config.setString("port", prefs.getString("tcpudp_port", "55435"));
			config.setString("host", prefs.getString("host_ip", "")); 
			config.setString("nickname", prefs.getString("nickname", "Player 1 Android"));
			config.setString("is_spectate","false");
			config.setInt("frame",Integer.valueOf(prefs.getString("delay_frame", "0"))); // prefs.getInt("delay_frame", 2));
				
		} else
			config.setBoolean("netplay_enable", false);
	...
	public void setCoreTitle(String core_name) {
		setTitle("RetroidNetplay : " + core_name);
	}
	...
	public void startActivity(Intent intent) {
		if (intent.getComponent().getClassName()
				.equals("org.retroidnetplay.browser.ROMActivity")) {
